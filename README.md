...
npm workspaces + [Turborepo](https://turbo.build/) of **shared NestJS libraries** for Nabarun APIs. This repository publishes `@ssdev-toolkit/*` packages; applications live in separate repos (for example the backend monorepo) and consume these libraries from npm or a workspace link.
...
Target one package with `npm run <script> -w @ssdev-toolkit/nestjs-shared-<name>`.

## Adding a shared package

Create `packages/nestjs-shared-<name>/package.json` (name `@ssdev-toolkit/nestjs-shared-<name>`), then depend on it from another workspace with `"*"` or from an app via npm. Turborepo runs dependency builds first via `dependsOn: ["^build"]`. After a public API or behavior change, run `npm run changeset`.
...
|--------|----------------|---------|
| `main` | `latest` | `npm install @ssdev-toolkit/nestjs-shared-core` |
| `stage` | `beta` | `npm install @ssdev-toolkit/nestjs-shared-core@beta` |
...
- Anyone can install without a token: `npm install @ssdev-toolkit/nestjs-shared-core` (stable) or `npm install @ssdev-toolkit/nestjs-shared-core@beta`.
