# `@ssdev-toolkit/comment-react`

React textarea editor and read-only mention rendering on top of `@ssdev-toolkit/comment-core`.

## Install

```bash
npm install @ssdev-toolkit/comment-core @ssdev-toolkit/comment-react
```
...
```tsx
import { MentionCommentEditor, CommentContent } from '@ssdev-toolkit/comment-react';
import { buildCommentEditorValue } from '@ssdev-toolkit/comment-core';
...
```bash
npm run build -w @ssdev-toolkit/comment-react
```
