...
```bash
npm install @ssdev-toolkit/auth-core @ssdev-toolkit/auth-angular
```
...
import { authGuard, permissionGuard } from '@ssdev-toolkit/auth-angular';
...
```bash
npm run build -w @ssdev-toolkit/auth-angular
npm test -w @ssdev-toolkit/auth-angular
```
